---
title: Article title
article_number: 1
article_slug: article-slug
permalink: /rd-ratchet/article-slug/
article_status: Draft
published: false
dek: One-sentence article promise.
date: 2026-01-01
updated: 2026-01-01
version: "0.1"
version_sequence: 1
revision_summary: Private working draft; not a public version
reading_time: 12
central_claim: The precise proposition this article tests.
claim_status: Provisional; evidence collection in progress
hero_image: /assets/images/rd-ratchet/rd-ratchet-hero.webp
hero_alt: Conceptual editorial illustration description.
hero_caption: Conceptual illustration; it does not depict a specific historical laboratory or event.
source_ids:
  - ncses-rd-2026
corrections: []
---

## Scene

Write a bounded firsthand moment. Record date, role, place, and the limits of what was personally visible.

## Incentive mechanism

State who paid, what return they could capture, the real time horizon, and what behavior the system rewarded.

## Institutional evidence

Separate documents, interviews, and later analysis. Attach evidence markers to contestable passages while drafting.

## Technical case

Trace one project from problem selection through research, engineering, transition, and what remained five years later.

## Strongest counterargument

Present the best alternative explanation before answering it.

## Five-part assessment

Evaluate scientific, technical, transition, institutional, and public-value outcomes separately.

## Implication for the successor

Identify the institutional design constraint learned from this case.

## Revision note

Before publication, state what changed relative to the previous public version. For version 1.0, use “Initial public version.” Accepted feedback belongs in the front-matter `corrections` ledger.
